...
BEGLOOP:
        ...
        if (DHASHING)
        {
                if (map->priv->array[BUCKETSIZ][index] >= (BUCKETSIZ>>1))
                        if (flag == INSERT)
        ...
        {
                if (map->priv->array[0][index] == (hashable_t) TOMBSTONE)
                {
                        if (flag == INSERT && !t)
                        {
                                t = 1;
                ...
                if ((hashable = map->priv->array[0][index]))
                {
                        if (obj->value != hashable->value) /* collision */
                        {
                                collisions++;
...
ENDLOOP:
        ...
        switch (flag)
        {
                case DELETE :
                        ...
                        ret = (u32)hashable;
                        map->priv->count--;
                        map->priv->array[BUCKETSIZ][index]--;
